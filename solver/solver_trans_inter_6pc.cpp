...
#include <Eigen/Dense>
#include "common.h"
#include "solver_trans_inter_6pc_core_48.h"
#include "solver_trans_inter_6pc_core_56.h"
...
Eigen::MatrixXcd solve_equation_inter_cam_48(double *input, double* zr, double* zi)
{
    ...
    Eigen::MatrixXcd sols = solver_trans_inter_6pc_core_48(data);
...
Eigen::MatrixXcd solve_equation_inter_cam_56(double *input, double* zr, double* zi)
{
    ...
    Eigen::MatrixXcd sols = solver_trans_inter_6pc_core_56(data);
    for (Index i = 0; i < sols.size(); i++) {
        zr[i] = sols(i).real();
        zi[i] = sols(i).imag();
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    {
        mexErrMsgIdAndTxt("JPL:tr_inter_6pc:notDouble", "Input data must be type double.");
    }

    bool is_known_angle = false;
    PC_TYPE pctype = PC_TYPE::PC_INTER_CAM_CONSTRAINT_FULL;
    if (nrhs >= 5)
    {
        double method = mxGetScalar(prhs[4]);
        if (method < -NEAR_ZERO_THRESHOLD || method > NEAR_ZERO_THRESHOLD)
        {
            pctype = PC_TYPE::PC_INTER_CAM_CONSTRAINT_PARTIAL;
        }
    }
    ...
    double *coeffs, *input;
    if (pctype == PC_TYPE::PC_INTER_CAM_CONSTRAINT_PARTIAL)
    {
